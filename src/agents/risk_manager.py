...
def risk_management_agent(state: AgentState):
    ...
    for ticker in all_tickers:
        ...
        # Fetch last 30 days of prices for risk calculations
        end_date = data["end_date"]
        start_date_var = (datetime.fromisoformat(end_date) - timedelta(days=30)).strftime("%Y-%m-%d")
        ...
        if not prices_df.empty:
            ...
            if prices_df["return"].dropna().empty:
                var_95 = 0.0
                drawdown = 0.0
            else:
                std_dev = prices_df["return"].std()
                var_95 = std_dev * 1.65 * current_price

                cumulative = (1 + prices_df["return"].fillna(0)).cumprod()
            ...
            risk_metrics[ticker] = {"var_95": 0.0, "max_drawdown": 0.0}
    ...
    for ticker in tickers:
        ...
        if ticker not in current_prices:
            ...
            risk_analysis[ticker] = {**risk_metrics.get(ticker, {}), "remaining_position_limit": 0.0, "current_price": 0.0, "reasoning": {"error": "Missing price data for risk calculation"}}
            continue
